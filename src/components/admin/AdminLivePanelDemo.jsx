...
// ── Score calculation (identical copy from AdminScoring — no import to avoid side effects) ─
function calculateScore(prediction, match) {
  ...
  if (pDir === aDir) {
    total += aDir === 'home' ? (match.home_win_points || 0)
  ...
  if (pRange === aRange) {
    total += aRange === '0-2' ? (match.goals_0_2_points || 0)
...
function findDbMatch(apiMatch, dbMatches) {
  ...
  return dbMatches.find(m =>
    (teamsMatch(h, m.team_a) || teamsMatch(hS, m.team_a)) &&
    (teamsMatch(a, m.team_b) || teamsMatch(aS, m.team_b))
  ) || null;
}
...
function deltaText(delta) {
  ...
  const places = n === 1 ? 'מקום אחד' : `${n} מקומות`;
  return `${dir} ${places}`;
}
      ...
      <div className="px-4 py-3">
        <div className="flex items-center justify-between mb-2">
          ...
          {match.utcDate && !isLive && (
            <span className="text-[10px] text-slate-500">{new Date(match.utcDate).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' })}</span>
          )}
        </div>
...
function RankCard({ row, index, total, shockwave }) {
  const delta   = row.officialRank - row.liveRank; // >0 = moved up
  const moved   = deltaText(delta);
  const cardDelay = Math.pow(total - 1 - index, 1.6) * 0.36 + (row.liveRank === 1 ? 0.5 : 0);
  const scoreDelay = cardDelay + 0.25;
  const arrowDelay = cardDelay + 0.5;
  const shockDelay = index * 0.22;
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}
      ...
      {/* cascade glow */}
      {shockwave && (
        <motion.div key={`glow-${row.userId}`} className="absolute inset-0 rounded-lg pointer-events-none"
          initial={{ opacity: 0 }} animate={{ opacity: [0, .22, 0] }}
          transition={{ duration: .55, delay: shockDelay, ease: 'easeOut' }}
          style={{ background: 'rgba(250,204,21,.22)', zIndex: 5 }} />
      )}

      {/* parallelogram card */}
      <div style={{ transform: 'skewX(-6deg)', borderRadius: 8, overflow: 'hidden', border: `2px solid ${border}`, background: RANK_BG(row.liveRank) }}>
        <div style={{ transform: 'skewX(6deg)', padding: '6px 12px' }}>
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="truncate text-xs font-semibold text-slate-200" style={{ animation: `lp-blur-focus 1.1s ease-out both`, animationDelay: `${cardDelay}s` }}>
                {row.name}
              </p>
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                {row.liveBonus > 0 && (
                  <motion.span initial={{ opacity: 0, scale: .7 }} animate={{ opacity: 1, scale: 1 }}
                    transition={{ delay: scoreDelay + .5, type: 'spring', stiffness: 350, damping: 18 }}
            ...
            <motion.div initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }}
              transition={{ delay: arrowDelay, duration: .4 }}
              className="flex flex-col items-center flex-shrink-0 w-14">
              {delta !== 0 ? (
                <>
                  <motion.span
                    initial={{ scale: .4, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                    transition={{ delay: arrowDelay + .1, type: 'spring', stiffness: 400, damping: 14 }}
                    style={{ fontSize: 22, lineHeight: 1, color: delta > 0 ? '#4ade80' : '#f87171' }}>
                    {delta > 0 ? '↑' : '↓'}
                  </motion.span>
                  <motion.span initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: arrowDelay + .25 }}
...
function LiveLBTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const uniqueStats = Object.values(statsMap).filter(s => {
        const n = nameMap[s.user_id];
        return n && !n.startsWith('user_');
      });
      ...
      const apiMatch = apiMatches[0] || null;
      ...
      if (apiMatch) {
        ...
        if (dbMatch) {
          ...
          liveMatchInfo = {
            home: apiMatch.homeTeam?.name || dbMatch.team_a,
            away: apiMatch.awayTeam?.name || dbMatch.team_b,
            score: `${hS}–${aS}`,
      ...
      // re-sort so cards animate in live rank order (for card reveal — last rank first)
      built.sort((a, b) => b.liveRank - a.liveRank);
      ...
      // trigger cascade glow after cards finish appearing
      const total = built.length;
      const glowAt = (Math.pow(total - 1, 1.6) * 0.36 + 0.5 + 1.4 + 0.5) * 1000;
      setTimeout(() => setShockwave(true), glowAt);

    } catch (e) {
      console.error('LiveLBTab error:', e);
      setStatus('error');
    }
  }, []);
  ...
  if (status === 'loading') {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3">
        ...
        <span className="text-slate-500 text-xs">טוען נתונים ומחשב לייב...</span>
      </div>
    );
  }
  if (status === 'error') {
  ...
  if (status === 'done' && rows.length === 0) {
    return <p className="text-center py-16 text-slate-500 text-sm">אין נתונים</p>;
  }
    ...
    <div>
      ...
      {liveInfo && (
        <div className="mb-3 px-3 py-2 rounded-xl flex items-center gap-2"
          ...
          <span className="text-emerald-400 text-xs font-bold">{liveInfo.home} {liveInfo.score} {liveInfo.away}</span>
          {liveInfo.minute && <span className="text-slate-500 text-[10px]">{liveInfo.minute}'</span>}
        </div>
      )}
      {!liveInfo && status === 'done' && (
        <div className="mb-3 px-3 py-2 rounded-xl flex items-center gap-2"
          style={{ background: 'rgba(100,116,139,.08)', border: '1px solid rgba(100,116,139,.25)' }}>
          <span className="text-slate-400 text-xs">אין משחק חי כרגע — מציג ניקוד רשמי</span>
        </div>
      )}

      {/* cards — animated in reverse rank order (last first, first last) */}
      <div>
        {rows.map((row, idx) => (
          <RankCard key={row.userId} row={row} index={idx} total={rows.length} shockwave={shockwave} />
        ))}
      </div>
...
// ── Match list tabs 1-3 ─────────────────────────────────────────────────────
const FILTER_MAP = { LIVE: 'LIVE', TODAY: 'TODAY', FINISHED: 'FINISHED' };
function MatchListTab({ filter }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const localDate = new Date().toLocaleDateString('sv-SE');
      const res  = await fetch(`/api/football?competition=WC&filter=${FILTER_MAP[filter]}&date=${localDate}`);
      const json = await res.json();
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="max-w-sm mx-auto rounded-2xl overflow-hidden"
        style={{ background: 'rgba(5,10,20,0.96)', border: '1px solid rgba(255,255,255,.1)', boxShadow: '-20px 0 60px rgba(0,0,0,.4)' }}>
